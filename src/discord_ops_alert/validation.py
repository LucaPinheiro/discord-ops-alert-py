...
class WebhookOptions(BaseModel):
    """Validated options for webhook mode."""
    ...
    def validate_webhooks(cls, v: dict[str, str]) -> dict[str, str]:
        for topic, url in v.items():
            if not url.startswith(_WEBHOOK_PREFIX):
                raise ValueError(
                    f"Invalid webhook URL for topic '{topic}': must start with {_WEBHOOK_PREFIX}"
                )
        return v
    ...
    def validate_default_username(cls, v: str | None) -> str | None:
        if v is not None and len(v) > _MAX_USERNAME_LENGTH:
            raise ValueError(
                f"default_username must be at most {_MAX_USERNAME_LENGTH} characters"
            )
        return v


class BotOptions(BaseModel):
    """Validated options for bot mode."""
    ...
    def validate_token(cls, v: str) -> str:
        if not _BOT_TOKEN_RE.match(v):
            raise ValueError(
                "Discord bot token must match the format: "
                "<24+ chars>.<6+ chars>.<27+ chars> (base64url+dots)"
            )
        return v
    ...
    def validate_channels(cls, v: dict[str, str]) -> dict[str, str]:
        for topic, channel_id in v.items():
            if not _SNOWFLAKE_RE.fullmatch(channel_id):
                raise ValueError(
                    f"Invalid channel_id for topic '{topic}': "
                    "must be 17-20 digits (Discord snowflake)"
                )
        return v


def validate_webhook_options(raw: dict) -> WebhookOptions:
    """Parse and validate webhook options. Raises DiscordOpsError(CONFIG_ERROR) on failure."""
    try:
...
def validate_bot_options(raw: dict) -> BotOptions:
    """Parse and validate bot options. Raises DiscordOpsError(CONFIG_ERROR) on failure."""
    try:
...
def validate_notify_input(input: NotifyInput) -> None:
    """Validate a NotifyInput before sending. Raises DiscordOpsError(VALIDATION_ERROR) on failure."""
    ...
    if len(input.message) > _MAX_MESSAGE_LENGTH:
        raise DiscordOpsError(
            ...
            f"message exceeds Discord's {_MAX_MESSAGE_LENGTH} character limit "
            f"({len(input.message)} chars)",
        )
